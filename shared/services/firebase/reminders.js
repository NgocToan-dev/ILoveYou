...
// NOTE: Notification service is platform-specific and should be injected
 ...
 * This should be called by platform-specific code (mobile/web) during app initialization
 * @param {Object} service - Platform-specific notification service
 */
export const initializeNotificationService = (service) => {
  notificationService = service;
  console.log('✅ Notification service initialized for reminders');
};
 ...
 * @returns {Promise<any>} - Result or null if service not available
 */
const safeNotificationCall = async (method, ...args) => {
  if (!notificationService || typeof notificationService[method] !== 'function') {
    console.warn(`⚠️ Notification service not available or method ${method} not found`);
  ...
  try {
    ...
    console.error(`Error calling notification service method ${method}:`, error);
...
export const createReminder = async (reminderData) => {
  try {
    ...
    try {
      const notificationResult = await safeNotificationCall('scheduleReminderNotification', createdReminder);
...
export const updateReminder = async (reminderId, updateData) => {
  try {
    ...
    try {
      ...
      await safeNotificationCall('cancelReminderNotifications', reminderId);
      ...
      if (!updatedReminder.completed && dueDate && dueDate > new Date()) {
        const notificationResult = await safeNotificationCall('scheduleReminderNotification', updatedReminder);
        if (notificationResult.success) {
...
export const deleteReminder = async (reminderId) => {
  try {
    ...
    try {
      const cancelResult = await safeNotificationCall('cancelReminderNotifications', reminderId);
...
// Re-export getCategoryDisplayInfo with specific name to avoid conflicts
export { getCategoryDisplayInfo as getReminderCategoryDisplayInfo };
