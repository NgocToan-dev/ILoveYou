...
const NotesScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    if (user) {
      console.log(
        "Setting up notes subscriptions for user:",
        user.uid,
        "coupleId:",
      ...
      if (userProfile?.coupleId) {
        console.log(
          "User has coupleId, subscribing to shared notes:",
          userProfile.coupleId
        );
        ...
        console.log("User has no coupleId, clearing shared notes");
        setSharedNotes([]);
      }
    }
  ...
  const loadCategoryCounts = async () => {
    ...
    try {
      ...
      if (userProfile?.coupleId) {
        const sharedCounts = await getCoupleNotesCountByCategory(
          userProfile.coupleId
        );
  ...
  const renderRecentNote = (note) => {
          const categoryInfo = getNoteCategoryDisplayInfo(note.category);
    ...
    return (
      ...
      >
        <View style={styles.noteHeader}>
          <View
            style={[styles.noteIcon, { backgroundColor: categoryInfo.color }]}
          >
  ...
  const recentNotes = searchTerm.trim()
    ? searchResults
    : currentNotes.slice(0, 5);
