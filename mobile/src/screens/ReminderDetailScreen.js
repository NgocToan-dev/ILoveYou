...
const ReminderDetailScreen = ({ navigation, route }) => {
      ...
      [
        { text: "Hủy", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            try {
              console.log(
                "Deleting reminder from detail screen:",
                reminder.id,
                reminder.title
              );
              await deleteReminder(reminder.id);
              console.log("Reminder deleted successfully from detail screen");
              Alert.alert("Thành công", "Đã xóa nhắc nhở.", [
                { text: "OK", onPress: () => navigation.goBack() },
              ]);
            } catch (error) {
              console.error(
                "Error deleting reminder from detail screen:",
                error
              );
  ...
  const handleShareReminder = async () => {
    try {
      const categoryInfo = getReminderCategoryDisplayInfo(reminder.category);
  ...
  if (reminder.dueDate && !reminder.completed) {
    try {
      ...
      console.error(
        "Error checking if reminder is overdue in detail screen:",
        error
      );
      isOverdue = false;
    }
            ...
            <View style={styles.metaInfo}>
              <View style={styles.metaRow}>
                <View style={styles.metaItem}>
                  <Ionicons
                    name={
                      reminder.type === REMINDER_TYPES.COUPLE
                        ? "people"
                        : "person"
                    }
                    size={16}
              ...
              <View style={styles.metaItem}>
                ...
                <Text
                  style={[styles.metaText, isOverdue && styles.overdueMetaText]}
                >
              ...
              <View style={styles.metaItem}>
                ...
                <Text style={styles.metaText}>
                  Tạo:
                  {formatDateString(reminder.createdAt, "relative", "vi-VN")}
                </Text>
              </View>
            </View>
